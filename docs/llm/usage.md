...
下载模型文件（本子模型文件名未： **llama2-7b-ggml-model-q4_0.bin** 为例，以实际你下载模型文件名为准）后，将模型放在插件的正确位置，我们提供了一个配置界面方便的完成这些操作：

依次在 Unreal Engine Editor 打开插件配置:
> 打开插件配置可以使用以下步骤：
> 1. 可以点击 `编辑（Edit）`>>`插件（Plugins）`。
> 2. 在`项目设置窗口（Project Settings）`左侧找到`插件(Plugins)`>>`MediaPipe4U LLM` 并点击。 
> 3. 找到数据目录中的`models`目录，将 **llama2-7b-ggml-model-q4_0.bin** 文件拷贝到`models`目录中。

[![Plugin Settings](./images/llm_open_data_folder.jpg "Plugin Settings")](./images/llm_open_data_folder.jpg)

在上述的设置界面中点击`Open LLM Data Folder` 按钮，你将打开一个 LLM 插件的数据目录。   

{: .important}
> 如果点击`Open LLM Data Folder`后没有自动打开目录，你可以手动找到它：   
> 目录路径是 `项目插件目录\MediaPipe4ULLM\Source\ThirdParty\LLMAPI\Data` 

[![Models Folder](./images/llm_models_folder.jpg "Models Folder")](./images/llm_models_folder.jpg)

将 **llama2-7b-ggml-model-q4_0.bin** 文件拷贝到 `models` 目录中以后，你还需要完成模型配置，模型配置文件是 LLM 数据目录下的 `llm.conf` 文件。   

用任意的文本编辑器（如：Windows 记事本）打开它，进行编辑:
...
默认值:**true**
{: .important}
> LLM 模型加载过程缓慢，为了避免卡住游戏线程, 加载是异步执行的。   
> 
...
**`SpeechSolution`**   
要使用的语音方案（适用于开发自定义 TTS 实现），关于自定义语言方案， 有专门文档说明。      
...
**`SpeechDelayCharNum`**   
当使用语音方案时，可能语音朗读速度和模型生成速度不匹配（CPU 推理时，模型生成较慢），如果直接朗读，可能造成语音中间有大量停顿。   
使用该属性延迟设置朗读延迟的字符数，只有基类的分句字符数大于`SpeechDelayCharNum`设置的字符数量时才开始朗读，这样可以对齐语音朗读和 LLM 生成的速度。   
...
{: .important}
> 这里事件名中的 "Token" 和 LLM 中 token 概念不太一样。   
> `OnLLMTokenGenerated`事件的token 其实是字符（一个英文字符/中文字符/标点符号），因为 MediaPipe4U 进行了分装，当多个 token 被合并成一个字符时才触发一次`OnLLMTokenGenerated`事件。
...
当 LLM 完整的生成了结果时触发。
> 触发该`OnCompletionFinished`事件时，表示 LLM 的一轮对话完成。   
...
如果为 **true**, 表示当前有聊天会话（已经开始了聊天），否则为 **false**。      
